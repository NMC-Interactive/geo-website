...
//    status | next | record | report | validate | list | init | help
//
//  Master-driven: every path comes from the master (playbook.yaml or
//  playbook.json) under `index` / `paths`, with sensible fallbacks. That means
//  this same file drops onto an existing project (even one using .json indexes)
//  and just works — the master tells it where everything lives.
//
//  Format-tolerant: index/config files may be .yaml or .json (js-yaml parses
//  both, since JSON is a subset of YAML). Everything resolves relative to the
//  master's folder, so the whole playbook is carry-on.
//
...
// ============================================================================
//  validate — lightweight, assert-and-collect guardrails (exit 1 on failure)
// ============================================================================
...
function cmdNext(args) {
  ...
  const candidate = tasks.filter((t) => t.status === 'todo').sort((a, b) => prio(a) - prio(b))[0];
...
function cmdRecord(args) {
  if (!args.task || !args.action || !args.status) {
    console.error('Usage: pb record --task <id> --action <action> --status <status> [--result <r>] [--files a,b] [--notes "..."] [--agent <name>]');
  ...
  console.log(`Recorded [${entry.task}] ${entry.action} → ${entry.status}`);
...
function cmdReport(args) {
  ...
  if (byTask.size === 0) {
    ...
    for (const [taskId, entries] of byTask) {
      ...
      for (const e of entries) {
        ...
        lines.push(`- \`${e.ts?.slice(0, 19)}\` **${e.action}** → ${e.status}${notes}${files}`);
      }
      lines.push('');
    }
  }
  ...
  if (open.length === 0) {
    ...
    lines.push('| Priority | ID | Status | Task | Skill |');
    lines.push('| --- | --- | --- | --- | --- |');
    for (const t of open) lines.push(`| ${prio(t)} | ${t.id} | ${t.status} | ${t.title} | ${t.skill || '-'} |`);
  }
  lines.push('');
...
function cmdInit() {
  ...
  if (!existsSync(p(BACKLOG))) {
    writeBacklog({ tasks: [{ id: 'T1', title: 'First task', status: 'todo', skill: 'run-task', priority: 1, notes: 'Replace me.', created: today() }] });
...
function cmdHelp() {
  ...
  Commands:
    ...
    record --task <id> --action <a> --status <s> [--result <r>] [--files a,b] [--notes "..."] [--agent <n>]
                           Append a structured entry to the journal (the agent-first record)
    report [--since DATE]  Roll the journal up into ${REPORTS_DIR}/report-<date>.md
    validate               Run the lightweight guardrails (exit 1 on failure)
