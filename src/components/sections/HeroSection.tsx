...
type Status = 'READY' | 'LIVE' | 'NEW'

function entryStatus(entry: ArticleEntry): Status {
  if (entry.pillar != null) return 'LIVE'
  if (entry.spoke != null) return 'READY'
  return 'NEW'
}
...
export default function HeroSection({ recentArticles }: Props) {
  ...
  // Derive terminal items from recentArticles, with 8-item fallback
  const terminalItems = recentArticles.length > 0
    ? recentArticles.slice(0, 8).map((a, i) => ({
    ...
    : [
        { id: '001', title: 'The Algorithmic Mind',           status: 'READY' as Status, url: '#', cluster: 'GEO' },
        { id: '002', title: 'Syntax & Semantics',             status: 'READY' as Status, url: '#', cluster: 'SEO' },
        { id: '003', title: 'Indexing Tomorrow',              status: 'LIVE'  as Status, url: '#', cluster: 'GEO' },
        { id: '004', title: 'Human Intent Patterns',          status: 'READY' as Status, url: '#', cluster: 'SEO' },
        { id: '005', title: 'Zero-Click Optimization',        status: 'NEW'   as Status, url: '#', cluster: 'GEO' },
        { id: '006', title: 'Entity Graph Protocol',          status: 'READY' as Status, url: '#', cluster: 'Resources' },
        { id: '007', title: 'Citation Algorithm Deep Dive',   status: 'LIVE'  as Status, url: '#', cluster: 'GEO' },
        { id: '008', title: 'Technical GEO Infrastructure',   status: 'READY' as Status, url: '#', cluster: 'Resources' },
      ]
                ...
                <div className="px-5 py-4" style={{ maxHeight: '420px', overflowY: 'auto' }}>
                  ...
                  {logsVisible >= logKeys.length && (
                    <div className="mb-3 flex items-center gap-4 font-mono text-xs" style={{ color: '#A0A0A0', letterSpacing: '0.06em' }}>
                      <span>{t('terminal.status.resources') as string}: <span style={{ color: '#3B82F6' }}>{terminalItems.length}</span></span>
                      <span>{t('terminal.status.live') as string}: <span style={{ color: '#10B981' }}>{terminalItems.filter(a => a.status === 'LIVE').length}</span></span>
                      <span>{t('terminal.status.new') as string}: <span style={{ color: '#F59E0B' }}>{terminalItems.filter(a => a.status === 'NEW').length}</span></span>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-1">
                    {terminalItems.slice(0, articlesVisible).map(article => (
                      <a
                        key={`${lang}-${article.id}`}
                        href={article.url}
                  ...
                  {articlesVisible >= terminalItems.length && (
